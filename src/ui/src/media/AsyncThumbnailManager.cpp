...
AsyncThumbnailManager::AsyncThumbnailManager(QObject *parent)
    : QObject(parent)
    ...
    , m_expiredWatcher(new QFutureWatcher<QPair<QPixmap, QString>>(this))
{
    ...
    connect(m_imageWatcher, &QFutureWatcher<QPair<QString, bool>>::finished,
            this, &AsyncThumbnailManager::onImageThumbnailFinished);

    connect(m_videoWatcher, &QFutureWatcher<QPair<QString, bool>>::finished,
            this, &AsyncThumbnailManager::onVideoThumbnailFinished);

    connect(m_expiredWatcher, &QFutureWatcher<QPair<QPixmap, QString>>::finished,
            this, &AsyncThumbnailManager::onExpiredThumbnailFinished);
}

AsyncThumbnailManager::~AsyncThumbnailManager()
{
    cancelAllTasks();
}

void AsyncThumbnailManager::setThumbnailStoragePath(const QString& path)
{
...
void AsyncThumbnailManager::generateImageThumbnailAsync(const QString& imagePath,
...
void AsyncThumbnailManager::generateVideoThumbnailAsync(const QString& videoPath,
                                                        const QString& thumbnailName,
                                                        const QSize& size)
{
    ThumbnailTask task;
...
void AsyncThumbnailManager::getExpiredThumbnailAsync(const QString& thumbnailName,
                                                     const QString& fileType,
                                                     bool checkSourceExists,
                                                     const QString& sourcePath)
{
    // 启动异步任务 - 使用 lambda 表达式
    QFuture<QPair<QPixmap, QString>> future = QtConcurrent::run([this, thumbnailName, fileType, checkSourceExists, sourcePath]() {
        return getExpiredThumbnailImpl(thumbnailName, fileType, checkSourceExists, sourcePath);
    });
    m_expiredWatcher->setFuture(future);
}

QPixmap AsyncThumbnailManager::getWarningThumbnail(const QString& thumbnailPath,const QString &fileType)
{
    if(!QFileInfo(thumbnailPath).exists()){
        return createDefaultExpiredThumbnail({100,100}, fileType);
    }
    return  createExpiredThumbnail(QPixmap(thumbnailPath), fileType);
}

bool AsyncThumbnailManager::thumbnailExists(const QString& thumbnailName)
{
    QString thumbnailPath = getThumbnailPath(thumbnailName);
    return QFileInfo(thumbnailPath).exists();
}

QString AsyncThumbnailManager::getThumbnailPath(const QString& thumbnailName)
{
    return m_thumbnailStoragePath + "/" + thumbnailName;
}

void AsyncThumbnailManager::setDefaultThumbnailSize(const QSize& size)
{
    if (size.isValid()) {
...
void AsyncThumbnailManager::cancelAllTasks()
{
    if (m_imageWatcher->isRunning()) {
...
int AsyncThumbnailManager::activeTaskCount() const
{
    int count = 0;
...
QString AsyncThumbnailManager::generateThumbnailName(const QString& originalPath, const QString& customName)
{
...
QPair<QString, bool> AsyncThumbnailManager::generateImageThumbnailImpl(const ThumbnailTask& task)
{
...
QPair<QString, bool> AsyncThumbnailManager::generateVideoThumbnailImpl(const ThumbnailTask& task)
{
...
QPair<QPixmap, QString> AsyncThumbnailManager::getExpiredThumbnailImpl(const QString& thumbnailName,
                                                                       const QString& fileType,
                                                                       bool checkSourceExists,
                                                                       const QString& sourcePath)
{
    ...
    if (checkSourceExists && !sourcePath.isEmpty()) {
        sourceExists = QFileInfo(sourcePath).exists();
    }
    ...
    if (thumbnailLoaded && !sourceExists) {
        QPixmap expiredThumbnail = createExpiredThumbnail(baseThumbnail, fileType);
        return qMakePair(expiredThumbnail, thumbnailName);
    }

    // 如果缩略图加载失败，创建默认的过期缩略图
    QPixmap expiredThumbnail = createDefaultExpiredThumbnail(m_defaultThumbnailSize, fileType);
    return qMakePair(expiredThumbnail, thumbnailName);
}

bool AsyncThumbnailManager::saveThumbnail(const QImage& image, const QString& thumbnailPath)
{
    if (image.isNull()) {
        return false;
    }
...
QPixmap AsyncThumbnailManager::createExpiredThumbnail(const QPixmap& baseThumbnail, const QString& fileType)
{
    ...
    QString text = QString("%1已过期或被清除").arg(fileType=="video"? "视频":"图片");
...
QPixmap AsyncThumbnailManager::createDefaultExpiredThumbnail(const QSize& size, const QString& fileType)
{
...
void AsyncThumbnailManager::onImageThumbnailFinished()
{
...
void AsyncThumbnailManager::onVideoThumbnailFinished()
{
...
void AsyncThumbnailManager::onExpiredThumbnailFinished()
{
